...
class GraphQLFactory {
    ...
    fun graphQL(resourceResolver: ResourceResolver, graphQLDataFetchers: GraphQLDataFetchers): GraphQL {
        ...
        val typeRepository = TypeDefinitionRegistry()
        val graphqkSchema = resourceResolver.getResourceAsStream("classpath:schema.graphqls")

        return if (graphqkSchema.isPresent) {
            typeRepository.merge(schemaParser.parse(BufferedReader(InputStreamReader(graphqkSchema.get()))))
            ...
            val graphQLSchema = schemaGenerator.makeExecutableSchema(typeRepository, runtimeWiring)
